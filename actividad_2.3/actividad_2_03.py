...
class OrangeDetector:
    ...
    def detect_orange_object(self, img):
        ...
        h_mask = cv2.inRange(h_channel, self.H_LOW, self.H_HIGH)
        # Saturation threshold: exclude low-sat pixels (floor/walls are near-grey)
        s_mask = cv2.inRange(s_channel, self.S_LOW, self.S_HIGH)
        ...
        # Best candidate = largest contour above minimum area
